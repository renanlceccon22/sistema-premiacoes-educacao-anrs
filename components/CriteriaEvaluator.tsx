...
const CriteriaEvaluator: React.FC<CriteriaEvaluatorProps> = ({
  ...
  const getAutoSelectedOptionId = (category: Category, realized: number, target: number) => {
    ...
    } else if (model === EvaluationModel.METRIC_ACCUMULATED) {
    ...
    <div className={`bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8 transition-all duration-500 ${isReadOnly ? 'bg-slate-50/50' : 'ring-1 ring-slate-100'}`}>
      ...
      <div className="space-y-6" ref={dropdownRef}>
        {localCategories.map((category) => {
          ...
          return (
            <div key={category.id} className="bg-slate-50/30 p-5 rounded-2xl border border-slate-100 last:mb-0 mb-6">
              {isEditMode ? (
                      ...
                      <div className="flex flex-col md:flex-row gap-4">
                        ...
                        <div className="flex-1">
                          <label className="text-[10px] font-black text-blue-400 uppercase tracking-widest leading-none mb-1.5 block">
                            {category.evaluationModel === EvaluationModel.METRIC_DIRECT ? 'Limite 1 (Melhor Faixa %)' : 'Delta 1 (Até % da Meta)'}
                          </label>
                          <div className="relative">
                            <input
                              type="number"
                        ...
                        <div className="flex-1">
                          <label className="text-[10px] font-black text-blue-400 uppercase tracking-widest leading-none mb-1.5 block">
                            {category.evaluationModel === EvaluationModel.METRIC_DIRECT ? 'Limite 2 (Faixa Média %)' : 'Delta 2 (Margem Aceitável %)'}
                          </label>
                          <div className="relative">
                            <input
                              type="number"
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-3 gap-3">
                  ...
                  {isMetricMode && (
                    <div className="flex items-center gap-3 bg-white p-2 rounded-2xl border border-slate-100 shadow-sm">
                      {(category.evaluationModel === EvaluationModel.METRIC_ACCUMULATED || category.evaluationModel === EvaluationModel.METRIC_RELATIVE) && (
                        <>
                          <div className="flex flex-col px-2">
                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">
                              {category.evaluationModel === EvaluationModel.METRIC_ACCUMULATED ? 'Meta Acumulada' : 'Meta Definida'}
                            </span>
                      ...
                      <div className="flex flex-col min-w-[140px] px-2">
                        ...
                        <div className="relative">
                          {category.id === 'inadimplencia_mes' ? (
                            <div className="px-2 py-1 bg-blue-50 text-blue-700 text-[10px] font-black rounded-lg border border-blue-100 uppercase tracking-tight flex items-center gap-1.5">
                            ...
                            <input
                              ...
                              className="w-full bg-white border border-slate-200 rounded-lg px-2 py-1.5 text-xs font-black text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#003B71]"
                            />
                          )}
                        </div>
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 mt-4">
                {category.options.map((option) => {
                  ...
                  return (
                    <button
                      key={option.id}
                      disabled={isReadOnly || isMetricMode}
                      onClick={() => !isReadOnly && !isMetricMode && onSelect(category.id, option.id)}
                      className={`flex justify-between items-center p-4 rounded-2xl border-2 transition-all duration-300 text-left relative overflow-hidden group ${isSelected
                        ...
                        } ${isReadOnly && !isSelected ? 'opacity-40 cursor-not-allowed' : ''} ${isMetricMode ? 'cursor-default' : ''}`}
                    >
                      {isSelected && (
